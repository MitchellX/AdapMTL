...
class MTSeqBackbone(nn.Module):
    ...
    # in this forward case, we need to duplicate the layers for n-task
    def forward(self, x): 
        ...
        if self.layout is None:
            ...
            for block in self.mtl_blocks:
                ...
                if block.layer_idx == self.layout.B - 1:    # self.layout.B indicate where the MTL will branch out
                    for task in block.task_set:     # block.task_set is used to determine the affiliation of this layer
                        features[task] = output     # if different task share the same layer, we can reuse the results of output feature
            return features
...
class MTSeqModel(nn.Module):
    def __init__(self, prototxt, backbone, layout=None, branch=None, fined_B=None, feature_dim=512, cls_num={}, backbone_init=None, heads_init=None, verbose=True):
    ...
    def forward(self, x):
        features = self.backbone(x)     # features of MTL, 3 separate outputs -> 1 output
        output = {}
        idx = 0
        for task in self.heads:         # len(self.heads) == len(tasks)
            output[task] = self.heads[task](features)
            idx += 1
        return output
